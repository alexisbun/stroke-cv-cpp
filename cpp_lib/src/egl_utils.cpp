...
PFNEGLGETNATIVECLIENTBUFFERANDROIDPROC fn_eglGetNativeClientBufferANDROID =
    nullptr;
...
EGLManager::EGLManager()
    : display_(EGL_NO_DISPLAY), config_(nullptr), context_(EGL_NO_CONTEXT),
      surface_(EGL_NO_SURFACE), nativeWindow_(nullptr),
      extensionsLoaded_(false) {}

EGLManager::~EGLManager() { ReleaseEGL(); }
...
bool EGLManager::InitializeEGL(ANativeWindow *window) {
  ...
  display_ = eglGetDisplay(
      EGL_DEFAULT_DISPLAY); // standard system graphics display driver
  if (display_ == EGL_NO_DISPLAY) {
    return false;
  }

  EGLint major, minor;
  if (!eglInitialize(display_, &major,
                     &minor)) { // initialize egl interface on the display
    return false;
  }

  const EGLint attributeList[] = {
      // frame buffer formating specification attributes
      EGL_RENDERABLE_TYPE,
  ...
  EGLint numConfigs;
  if (!eglChooseConfig(display_, attributeList, &config_, 1, &numConfigs) ||
      numConfigs < 1) {
    return false;
  }
  ...
  context_ =
      eglCreateContext(display_, config_, EGL_NO_CONTEXT, contextAttributes);
  if (context_ == EGL_NO_CONTEXT) {
    return false;
  }

  surface_ =
      eglCreateWindowSurface(display_, config_, nativeWindow_, nullptr); //
...
GLuint EGLManager::InitGLExternalTexture() {
  ...
  glBindTexture(GL_TEXTURE_EXTERNAL_OES,
                textureId); // bind external texture to current texture object
...
EGLImageKHR EGLManager::BindHardwareBuffer(AHardwareBuffer *buffer,
                                           GLuint textureId) {
  ...
  if (it != eglImageCache_.end()) {
    ...
    image = fn_eglCreateImageKHR(display_, EGL_NO_CONTEXT,
...
bool EGLManager::InitShaders() {
  ...
  glShaderSource(pointFragmentShader, 1, &Shaders::POINT_FRAGMENT_SOURCE,
                 nullptr);
  ...
  glBufferData(GL_ARRAY_BUFFER, 478 * 2 * sizeof(float), nullptr,
               GL_DYNAMIC_DRAW);
  glEnableVertexAttribArray(0);
  ...
  glShaderSource(strokeVertexShader, 1, &Shaders::STROKE_VERTEX_SOURCE,
                 nullptr);
  glCompileShader(strokeVertexShader);

  GLuint strokeFragmentShader = glCreateShader(GL_FRAGMENT_SHADER);
  glShaderSource(strokeFragmentShader, 1, &Shaders::STROKE_FRAGMENT_SOURCE,
                 nullptr);
  ...
  strokeTextureUniformLocation_ =
      glGetUniformLocation(strokeProgramId_, "u_texture");

  strokeDeltaTextureLoc_ =
      glGetUniformLocation(strokeProgramId_, "u_deltaTexture");
  ...
  glBufferData(GL_ARRAY_BUFFER, 478 * 4 * sizeof(float), nullptr,
               GL_DYNAMIC_DRAW);

  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, strokeEbo_);
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(FACE_MESH_TRIANGLES),
               FACE_MESH_TRIANGLES, GL_STATIC_DRAW);
...
void EGLManager::DrawLandmarks(const std::vector<float> &projectedCoordinates) {
  ...
  glBufferSubData(GL_ARRAY_BUFFER, 0,
                  projectedCoordinates.size() * sizeof(float),
                  projectedCoordinates.data());
  // Render as points
  glDrawArrays(GL_POINTS, 0, projectedCoordinates.size() / 2);
  glBindVertexArray(0);
}

void EGLManager::UploadDeltaTexture(const uint8_t *rgbBytes256) {
  if (!rgbBytes256 || deltaTextureId_ == 0)
    return;
  glBindTexture(GL_TEXTURE_2D, deltaTextureId_);
...
void EGLManager::DrawStrokeEffect(const std::vector<float> &meshVertexData,
  ...
  glBindTexture(GL_TEXTURE_2D, deltaTextureId_);
  glUniform1i(strokeDeltaTextureLoc_, 1);
  ...
  glBindVertexArray(strokeVao_);
  glBindBuffer(GL_ARRAY_BUFFER, strokeVbo_);
  glBufferSubData(GL_ARRAY_BUFFER, 0, meshVertexData.size() * sizeof(float),
                  meshVertexData.data());
